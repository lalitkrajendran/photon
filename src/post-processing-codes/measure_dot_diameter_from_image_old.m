function dot_diameters = measure_dot_diameter_from_image(method, img_filepath, read_order, particle_id_filepath, particle_size_filepath)
...
dot_diameters = zeros(num_analysis,1);
...
for i = 1:1+skip:N
    ...
    if strcmp(method, 'imfindcircles')
        ...
        dot_diameters(image_ctr) = diameter;
        
%         % save results to file
%         save([particle_id_filepath 'img_' num2str(image_ctr, '%02d') '.mat'], 'centers'); 
%         save([particle_size_filepath 'img_' num2str(image_ctr, '%02d') '.mat'], 'diameter');
