...
for grad_x_index = 1:num_cases_grad_x
    for seeding_density_index = 1:num_cases_seeding_density
        ...
        for file_index = 1:num_files
            ...
            U_temp_valid = U_temp(abs(U_temp - U_ref_array(grad_x_index)) < valid_vector_threshold);
            V_temp_valid = V_temp(abs(V_temp - V_ref_array(grad_x_index)) < valid_vector_threshold);
            ...
            [err_U_bias_single(file_index), err_U_random_single(file_index), ...
                err_U_total_single(file_index)] = compute_errors(U_temp_valid(:)', U_ref_array(grad_x_index));
            [err_V_bias_single(file_index), err_V_random_single(file_index), ...
                err_V_total_single(file_index)] = compute_errors(V_temp_valid(:)', V_ref_array(grad_x_index));
...
[X,Y] = meshgrid(seeding_density_array, U_ref_array);
