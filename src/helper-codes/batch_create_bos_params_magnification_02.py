...
# set filepath
filepath = '/home/barracuda/a/lrajendr/Projects/camera_simulation/data/bos_parameters/magnification/'
...
focal_length_array = np.linspace(start=100e3, stop=300e3, num=9, endpoint=True)
...
for grad_x_index in range(0,grad_x_array.size):
    ...
    for i in range(0,focal_length_array.size): #len(dot_size_microns)):
        ...
        x_pixel_number_dots = int(512 * focal_length_array[i] / 200.0e3)
        y_pixel_number_dots = int(512 * focal_length_array[i] / 200.e3)
        ...
        piv_simulation_parameters['lens_design']['focal_length'] = focal_length_array[i] #200e3 #105e3
        ...
        # set the top directory which contain images of all magnifications
        ...
        top_write_directory = '/home/barracuda/a/lrajendr/Projects/camera_simulation/results/images/bos/error-analysis/magnification/'
        ...
        for image_pair_index in range(0,number_of_image_pairs):
            ...
            piv_simulation_parameters['output_data']['bos_pattern_image_directory'] = top_write_directory + 'grad_x=%.2f/%d/%d/' % \
                                                                                      (grad_x, focal_length_array[i]*1e-3, offset + image_pair_index+1)
            ...
            filename_full = filename_base + '%04d' % (grad_x_index*focal_length_array.size*number_of_image_pairs + i*number_of_image_pairs+image_pair_index+1) + '.mat'
            # filename_full = filename_base + '%04d' % (i*number_of_image_pairs + image_pair_index+1) + '.mat'
