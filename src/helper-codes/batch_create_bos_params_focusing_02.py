...
image_plane_perturbation_array = np.linspace(-0.5, 0.5, num=51, endpoint=True)
...
for image_plane_perturbation_index in range(0,len(image_plane_perturbation_array)):
    ...
    for image_pair_index in range(0,number_of_image_pairs):
        ...
        piv_simulation_parameters['output_data']['bos_pattern_image_directory'] = top_write_directory + 'grad_x=%.2f/perturbation=%d/%d/' % \
                                                                                  (grad_x, image_plane_perturbation*100, offset + image_pair_index+1)
