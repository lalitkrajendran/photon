...
dot_size_microns_array = np.linspace(start=200,stop=1000, num=18, endpoint=True)
...
for grad_x_index in range(0,grad_x_array.size):
    ...
    for i in range(0,dot_size_microns_array.size): #len(dot_size_microns)):
        ...
        for image_pair_index in range(0,number_of_image_pairs):
            ...
            piv_simulation_parameters['output_data']['bos_pattern_image_directory'] = top_write_directory + 'grad_x=%.2f/%dum/%d/' % \
                                                                                      (grad_x, dot_size_microns_array[i], offset + image_pair_index+1)
            ...
            filename_full = filename_base + '%04d' % (grad_x_index*dot_size_microns_array.size*number_of_image_pairs + i*number_of_image_pairs+image_pair_index+1) + '.mat'
